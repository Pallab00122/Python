...
class ChatClient(QWidget):
    def __init__(self):
        ...
        self.receiver_thread = threading.Thread(target=self.background_task, daemon=True)
        self.receiver_thread.start()

    def send_message(self):
        message = self.input_box.text()
        if message:
            client_port=self.client_socket.getsockname()
            self.client_socket.send(message.encode())
            self.chat_box.append(f"[port : {client_port} ] You:  {message}")
